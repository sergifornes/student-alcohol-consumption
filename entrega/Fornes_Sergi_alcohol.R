...
# Se colapsan las variables espec铆ficas de cada asignatura y se arreglan las dem谩s variables
data <- data %>%
...
ggplot(data) +
  ...
  ggtitle("Distribuci贸n de los estudiantes por consumo de alcohol") +
...
# Estimaci贸n del modelo con Leave-One-Out Cross-Validation
...
# Se crea una variable para guardar la precisi贸n del modelo
rf_acc <- 0
# Se especifican los par谩metros que se usar谩n para estimar los modelos
...
kable(xgb_tune, align = c("c","c","c","c","c","c","c","c")) %>%
  kable_styling()
# Se estima el modelo con los hiperpar谩metros 贸ptimos y los datos train
...
# Se estima el modelo con el hiperpar醡etro 髉timo y todos los datos
...
kable(arrange(tibble(Variable = row.names(fitRfFinal$importance),
                     `Disminuci髇 Media de GINI` = round(fitRfFinal$importance, 3)),
              desc(`Disminuci髇 Media de GINI`)),
      align = c("l", "c")) %>%
  kable_styling()